...
//   u32 nImages  { u16 w, u16 h, u8 channels(4), u8 wrapS, u8 wrapT, u8 linear (v4+; were pad),
...
//
// Standalone image files (".eimg", terrain satellite tiles): magic "EIMG" u32 version(1) + one v5 image record.
#pragma once
...
constexpr uint32_t EMOD_VERSION = 5;   // v1 (no node extras), v2 (TEXCOORD_0 only), v3 (material UV set), v4 (image
                                       // wrap/colour-space flags, emissive strength, UV transforms baked) still load;
                                       // v5 = images as GPU-format variants with mip chains (ETC2 / BC / RGBA8)
