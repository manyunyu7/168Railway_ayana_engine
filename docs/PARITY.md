...
|---|---|---|
| Bézier track, arc-length LUT, vertical profile, rails/ballast, bridges (deck+piers), tunnels | ✅ | `track_graph`, `rail_profile`, `rail_builder`. Corridor atlas: the catalog's `tekstur.rel1067` picture (`pilot-rel-1067.jpg`, pilot asset, not in the repo) converted by `tools/imgconv` into `build/assets/cache/*.eimg` (`RailBuilder::prepareAtlas/loadAtlas`, flipped like a three.js texture, clamp S / repeat T); the procedural painter (same layout) is the fallback. `RailBuilder::setAtlas(rhi::Texture)` for the web adapter (not wired yet). Materials as `dunia3d.ts:1302-1303` / `bangun3d.ts:82` (ballast roughness 1, rails 0.3/0.42, concrete 0x9aa0a6, tunnel 0x4a4d52 double-sided, steel 0x5c6b5a). Tunnel mouths: engine addition — the carve corridor narrows to 4.5/13 m at the mouth (instead of fading to 0) and the first 12 m inside the portal are registered, so the portal ring is not buried when the z13 DEM sits above the rail (`railtest ENG_TARGET=mouth:n`) |
